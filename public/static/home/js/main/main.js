...
require(['jquery', 'common', 'jquerySuperSlide', 'highstock', 'placeholder'], function($) {
    ...
    jQuery(".slideBox1 .bnr-lst,.bnr-btn").hover(function() {
        $('.bnr-btnfl,.bnr-btnfr').css({
            visibility: 'visible'
        });
    }, function() {
        $('.bnr-btnfl,.bnr-btnfr').css({
    ...
    $(window).resize(function() {
        $('.bnr-lst,.bnr-lst li').width($(window).width())
    });
    jQuery(".slideBox2").slide({
        ...
        autoPlay: true,
        scroll: "1",
        titCell: ".rec-ciecle i",
        interTime: "4000",
        delayTime: 600,
        trigger: "click"
    });
    jQuery(".ztscroll").slide({
        mainCell: ".bd ul",
        effect: "left",
        autoPlay: true,
        delayTime: 500,
    ...
    $(document).ready(function() {
        ...
        if ($('.ind-img').length > 0 && netRevenueData) {
            drawChart(netRevenueData, {
                'renderTarget': $('.ind-img')[0],
                'imageX': 270,
                'text': '暂时无数据',
                'px': 2,
                'height': 310
    ...
    function drawChart(chartData, drawOptions) {
        if (chartData[0] && chartData[0] && chartData[0].data && chartData[0].data.length == 0) {
            ...
            var chart = new Highcharts.StockChart({
                ...
                tooltip: {
                    ...
                    formatter: function() {
                        ...
                        if (!chartData[0]['flag']) {
                            ...
                            var header = '<b>时间: ' + date.getFullYear() + '年' + (date.getMonth() + 1) + '月' + '</b>';
                        }
                ...
                //TODO 加入背景图片
                oChart.renderer.image('./img/highcharts-logo.png', imageX, imageY, 248, 55).add();
